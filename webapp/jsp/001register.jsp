...
<body>
	<div class=container>
		...
		<main>
			<h1>会員登録</h1>
			<form id="register" method="post" action="<%=request.getContextPath()%>/MemberRegistrationConfirmation">
				<dl>
					<dt>氏名</dt>
					<dd>
					...
					<dt>氏名（フリガナ）</dt>
					<dd>
						セイ<input type="text" name="hurigana_sei" id="hurigana_sei" maxlength="30" required> 
					...
					<%if (sum % 5 == 0) {%>
					<dt></dt>
					<dd class="attention">＊適当なIDを入力してください</dd>
					<%}%>
					<%if (sum % 7 == 0) {%>
					<dt></dt>
					...
					<dd>
						<input type="password" name="password" id="password" maxlength="8"
							placeholder="半角英数字8ケタ" pattern="[a-zA-Z0-9]+"
							title="パスワードは半角英数字で入力してください。" required class="left">
					</dd>
					<dt>パスワード(再入力)</dt>
					<dd>
						<input type="password" name="repassword" id="repassword"
							maxlength="8" placeholder="半角英数字8ケタ" pattern="[a-zA-Z0-9]+"
							title="パスワードは半角英数字で入力してください。" required class="left">
					</dd>
					<%if (sum % 2 == 0) {%>
					<dt></dt>
		...
		<footer>
			<small> &copy;2023 Special Willingness And Travels. </small>
		</footer>
	</div>
</body>
</html>